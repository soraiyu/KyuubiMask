...
PRを作成する際は、`CHANGELOG.md` の `[Unreleased]` セクションに変更内容を追記してください。  
追記しない場合でも、PRテンプレートの **Changelog** セクションに内容を記載すれば、リリース時に自動でCHANGELOGに反映されます。  
どちらも書かない場合は、PRのタイトルがCHANGELOGエントリとして自動使用されます。
