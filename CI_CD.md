...
**トリガー条件**:
- `main`, `master`, `develop` ブランチへのpush（`app/` または `security-check.yml` の変更時）
- `main`, `master`, `develop` ブランチへのPull Request

**実行内容**:
1. ✅ コードのチェックアウト
2. 🔒 INTERNET 権限がないことを確認
3. 🔒 `allowBackup="false"` の確認
4. 🔒 リリースビルドで ProGuard/R8 の難読化が有効なことを確認
5. 🔒 危険な権限（位置情報、カメラ、連絡先など）がないことを確認
6. 🔒 `Log.*` の呼び出しがすべて `BuildConfig.DEBUG` ガードで保護されていることを確認
7. 🔒 ソースコードにハードコードされた秘密情報やAPIキーがないことを確認

**目的**:
プライバシーとセキュリティの要件が継続的に満たされていることを自動的に検証します。

### 3. Auto Label PR (auto-label.yml)

**トリガー条件**:
- `main` または `master` ブランチへのPull Requestが開かれた時、または更新された時

**実行内容**:
- PRで変更されたファイルを確認し、アプリに影響するファイル（`app/`、`build.gradle.kts`、`gradlew` など）が含まれていない場合、自動的に `skip-release` ラベルを付与する
- アプリに影響するファイルが含まれている場合、`skip-release` ラベルを削除する

アプリに影響しないファイルの例:
- `.github/workflows/` — GitHub Actions ワークフロー
- `*.md` — ドキュメント (README, CHANGELOG など)
- `LICENSE`, `SECURITY.md` など

### 3. Auto Tag on PR Merge (auto-tag.yml)

**トリガー条件**:
- `main` または `master` ブランチへのPull Requestがマージされた時
- ただし、以下のラベルが付いたPRはスキップされる:
  - `skip-release`
  - `fdroid`
  - `release`（リリースPR自身のマージはPhase 2で処理される）

**バージョン更新のスキップ**:

アプリのバージョンに関係ない変更（GitHub Actionsの更新、ドキュメントの修正など）は、PRに `skip-release` ラベルを付けることでバージョンバンプをスキップできます。このラベルは `auto-label.yml` ワークフローによって自動的に付与されます。手動で付与・削除することも可能です。

**実行内容（2フェーズ）**:

**Phase 1 — リリースPR作成** (通常のPRマージ時):
1. ✅ コードのチェックアウト
2. 📝 `app/build.gradle.kts` から `versionName` と `versionCode` を抽出
3. 🔢 同じタグまたは同名のリリースブランチが既に存在する場合、パッチバージョンと `versionCode` を自動インクリメント（`app/build.gradle.kts` も更新）
4. 📝 CHANGELOG.md の `[Unreleased]` セクションをバージョン付きセクションに変換
5. 📝 fastlane チェンジログを生成
6. 🌿 `release/vX.Y.Z` ブランチを作成してpush
7. 🔖 `release` ラベル付きのリリースPRを自動作成

**Phase 2 — タグ作成** (リリースPRマージ時):
1. ✅ `main` をチェックアウト
2. 🏷️ バージョンタグ（例: `v1.8.0`）を作成してpush
3. タグのpushにより、自動的に Release Build ワークフローが起動

**メリット**:
- `main` へ直接pushしないため、**ブランチ保護ルールのバイパスが不要**
- `versionName` を手動で更新する必要がない（自動インクリメント）
- 手動でタグを作成する必要がない

**一回限りのセットアップ**:

GitHub Actions がPRを作成できるよう設定が必要です（ブランチ保護のバイパスは不要）。

> Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests" にチェックを入れる
...
[![Security Check](https://github.com/soraiyu/KyuubiMask/actions/workflows/security-check.yml/badge.svg)](https://github.com/soraiyu/KyuubiMask/actions/workflows/security-check.yml)
...
PRを `main` ブランチにマージするだけで、自動的にリリースが準備されます：

#### 自動リリース（推奨）

1. 機能追加・バグ修正などの変更をコミットしてPRを作成
2. PRを `main` ブランチにマージ
3. 自動的に `release/vX.Y.Z` ブランチとリリースPRが作成される
4. リリースPRをマージ

これだけで自動的に：
- ✅ タグが既に存在する場合、`versionName` と `versionCode` がパッチインクリメントされる（例: `1.7.0` → `1.7.1`）
- ✅ CHANGELOG.md の `[Unreleased]` がバージョン付きセクションに変換される
- ✅ バージョンタグ（例: `v1.7.1`）が作成される
- ✅ Release APK が自動ビルドされる
- ✅ GitHub Release が作成される
- ✅ APK がリリースページに添付される

> **補足**: PRの中で `app/build.gradle.kts` の `versionName` を手動で変更した場合（例: マイナーバージョンアップ `1.7.0` → `1.8.0`）、その値がそのまま使われます。タグが既に存在する場合のみ自動インクリメントが行われます。
...
- `app/build.gradle.kts` の `versionName` がセマバー形式（`x.y.z`）か確認
- GitHub Actions のログを確認して、エラーメッセージを確認
- **リリースPRが作成されない場合**: Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests" が有効か確認
...
1. 機能追加・バグ修正のPRを作成して `main` にマージ
2. 自動的にバージョンが更新・タグが作成され、Release APK がビルドされるのを待つ
3. Releases ページから署名済み APK をダウンロードして実機でテスト
