...
3. 🔢 同じタグが既に存在する場合、パッチバージョンと `versionCode` を自動インクリメント（`app/build.gradle.kts` も更新）
4. 📝 CHANGELOG.md の `[Unreleased]` セクションをバージョン付きセクションに変換
5. 📝 fastlane チェンジログを生成
6. 🚀 バージョンファイルの変更があれば `main` に直接コミット & プッシュ
7. 🏷️ バージョンタグ（例: `v1.8.0`）を作成してpush
8. タグのpushにより、自動的に Release Build ワークフローが起動

**メリット**:
- PRをマージするだけで即座にリリースが完了する（中間PRなし）
- `versionName` を手動で更新する必要がない（自動インクリメント）
- 手動でタグを作成する必要がない

**一回限りのセットアップ**:

GitHub Actions が `main` ブランチに直接プッシュできるよう設定が必要です。

> **ブランチ保護ルールを設定している場合**:
> Settings → Rules → Rulesets → （mainのルールセット） → Bypass list → 「GitHub Actions」を追加
>
> **個人リポジトリでブランチ保護を使っていない場合**: 追加設定不要。
...
- **push 失敗の場合**: GitHub Actions が `main` ブランチに直接プッシュする権限があるか確認
  - ブランチ保護ルールを使っている場合 → Settings → Rules → Rulesets → mainのルールセット → Bypass list に「GitHub Actions」を追加
  - 個人リポジトリでブランチ保護なし → 追加設定不要
