...
3. 🔢 同じタグが既に存在するか、処理中のリリースブランチが既に存在する場合、パッチバージョンと `versionCode` を自動インクリメント（`app/build.gradle.kts` も更新）
4. 📝 CHANGELOG.md / F-Droid メタデータ / fastlane チェンジログを更新してコミット
5. 🏷️ 新しいバージョンタグを作成してpush
6. 🚀 タグがpushされると、自動的に Release Build ワークフローが起動

**メリット**:
- PRをマージするだけで自動的にバージョンが更新され、リリースが作成される
...
PRを `main` ブランチにマージするだけで、自動的にリリースが作成されます：
